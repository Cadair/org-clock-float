...
Only tasks tagged with a matching tag are uploaded to float. The format of the tag is ~float_<project name>~ or ~floatid_<project code>~ so if you have a project in float named "client1" you would have a task which looks like this:
...
of if you have a project with a long name and a code of 314 you would have this:

#+begin_src org
  * TODO Work on thing :floatid_134:
#+end_src
